...
@section('title') Edit Kegiatan Harian @endsection
@section('content')
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">Edit Kegiatan Harian</h4>
        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body p-4">
                <form action="{{ route('daily-activities.update', $dailyActivity->id) }}" method="POST">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <label class="form-label fw-bold"><i class="ri-calendar-event-line text-info me-1"></i> Tanggal Kegiatan <span class="text-danger">*</span></label>
                            <input type="date" name="activity_date" class="form-control form-control-lg @error('activity_date') is-invalid @enderror" value="{{ old('activity_date', $dailyActivity->activity_date->format('Y-m-d')) }}" required>
                            @error('activity_date')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                            @enderror
                        </div>
                        <div class="col-md-6 mt-3 mt-md-0">
                            <label class="form-label fw-bold"><i class="ri-stethoscope-line text-info me-1"></i> Tindakan Medis <span class="text-danger">*</span></label>
                            <select class="form-select form-select-lg @error('medical_activity_id') is-invalid @enderror" name="medical_activity_id" required>
                                ...
                                @foreach($medicalActivities as $ma)
                                    <option value="{{ $ma->id }}" {{ old('medical_activity_id', $dailyActivity->medical_activity_id) == $ma->id ? 'selected' : '' }}>
                                        {{ $ma->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="card border border-light bg-light shadow-none mb-4">
                        <div class="card-body">
                            <h5 class="font-size-15 mb-3 text-info"><i class="ri-user-heart-line me-1"></i> Data Pasien</h5>
                                ...
                                <div class="position-relative">
                                    <div class="input-group">
                                        ...
                                        <input 
                                            ...
                                            autocomplete="off"
                                            value="{{ old('patient_name', $dailyActivity->patient_name) }}"
                                            required>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3 mb-md-0">
                                    ...
                                    <input type="text" name="patient_name" id="patient_name" class="form-control bg-white @error('patient_name') is-invalid @enderror" value="{{ old('patient_name', $dailyActivity->patient_name) }}" readonly required tabindex="-1">
                                    @error('patient_name')
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="medical_record_no" id="medical_record_no" class="form-control bg-white @error('medical_record_no') is-invalid @enderror" value="{{ old('medical_record_no', $dailyActivity->medical_record_no) }}" readonly required tabindex="-1">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-bold"><i class="ri-file-text-line text-info me-1"></i> Keterangan / Catatan Tambahan</label>
                        <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="5" placeholder="Tulis catatan, instruksi, atau observasi tambahan di sini (opsional)...">{{ old('notes', $dailyActivity->notes) }}</textarea>
                        @error('notes')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                    </div>
